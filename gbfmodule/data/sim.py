...
class GBFSim:
    ...
    def _generate_summon_stat(self):
        ...
        for filename in summon_json_filename_list:
            ...
            summon_info = get_summon_info_from_filename(filename)
    ...
    def _summon_db_add_item_by_filename(self, item_filename):
        self._summon_db_add_item(get_summon_info_from_filename(item_filename))
...
# 从文件名获取召唤石信息
def get_summon_info_from_filename(filename):
    find = re.findall(r'((\d)0(\d)0(\d{3})000)\.json', filename)
    if not find:
        return None
    info_tuple = find[0]
    if info_tuple[1] != '2':
