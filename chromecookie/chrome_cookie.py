...
import json
import base64
import chromecookie.aesgcm as aesgcm
...
FROM
    cookies
WHERE
    host_key LIKE '%game.granbluefantasy.jp%'
"""
...
class ChromeCookieJar(cookiejar.FileCookieJar):
    def __init__(self, filename=None, delayload=False, policy=None, url=None):
        self._url = url
    ...
    def _really_load(self, f, filename, ignore_discard, ignore_expires):
        ...
        cur = con.cursor()
        cur.execute("SELECT host_key, name, path, is_secure, expires_utc, encrypted_value as value FROM cookies WHERE host_key = '{}'".format(self._url))
        for row in cur:
